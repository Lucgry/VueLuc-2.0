// ✅ REEMPLAZÁ tu CostSummary.tsx por este (solo cambia lo necesario: normalización y summary de métodos)
...
/**
 * ✅ Normaliza SOLO a estas 5 categorías:
 * - Débito Macro
 * - Débito Ciudad
 * - Crédito Macro
 * - Crédito Ciudad
 * - Crédito Yoy
 *
 * Todo lo demás => "Otros" (y luego lo filtramos para que no aparezca)
 */
const normalizePaymentMethod = (paymentMethod: string | null): string => {
  if (!paymentMethod) return 'Otros';

  const raw = String(paymentMethod).trim();
  const s = raw.toLowerCase();

  // --- Map por últimos 4 / tokens que ya venías usando ---
  // (Esto cubre "XXXX-6007" o strings similares)
  if (raw.includes('6007') || s.includes('macro') && s.includes('deb')) return 'Débito Macro';
  if (raw.includes('9417') || s.includes('ciudad') && s.includes('deb')) return 'Débito Ciudad';
  if (raw.includes('5603') || s.includes('macro') && (s.includes('cred') || s.includes('crédito') || s.includes('credito')))
    return 'Crédito Macro';
  if (raw.includes('8769') || s.includes('ciudad') && (s.includes('cred') || s.includes('crédito') || s.includes('credito')))
    return 'Crédito Ciudad';
  if (raw.includes('8059') || s.includes('yoy') && (s.includes('cred') || s.includes('crédito') || s.includes('credito')))
    return 'Crédito Yoy';

  // --- Si viene ya normalizado exactamente ---
  ...
  // Ejemplos de "basura" típica que NO queremos mostrar
  // - "Debito"
  // - "Tarjeta de crédito"
  // - "Débito Nación"
  // - etc.
...
const CostSummary: React.FC<CostSummaryProps> = ({ trips }) => {
  ...
  // ✅ CORREGIDO: agrupa solo en 5 categorías, filtra "Otros"
  const paymentMethodSummary = useMemo(() => {
    const costsByMethod: Record<string, number> = {};

    for (const trip of tripsForSelectedYear) {
      const flights = [trip.departureFlight, trip.returnFlight].filter(Boolean) as Flight[];

      for (const flight of flights) {
        const cost = flight.cost || 0;
        if (cost <= 0) continue;

        const method = normalizePaymentMethod(flight.paymentMethod);
        if (method === 'Otros') continue;

        costsByMethod[method] = (costsByMethod[method] || 0) + cost;
      }
    }
