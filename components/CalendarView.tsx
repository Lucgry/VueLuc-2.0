...
const CalendarView: React.FC<{ flightLegs: FlightLeg[] }> = ({ flightLegs }) => {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [viewMode, setViewMode] = useState<'agenda' | 'grid'>('agenda');
  const [tooltip, setTooltip] = useState<{ flights: FlightLeg[]; position: { top: number; left: number } } | null>(null);
  ...
  const flightsInMonth = useMemo(() => {
    const flightsMap: { [day: number]: FlightLeg[] } = {};
    const year = currentDate.getFullYear();
    const month = currentDate.getMonth();

    flightLegs.forEach(leg => {
      if (leg.departureDateTime) {
        const flightDate = new Date(leg.departureDateTime);
        if (flightDate.getFullYear() === year && flightDate.getMonth() === month) {
          const day = flightDate.getDate();
          if (!flightsMap[day]) flightsMap[day] = [];
          flightsMap[day].push(leg);
        }
      }
    });

    for (const day in flightsMap) {
        flightsMap[day].sort((a, b) => new Date(a.departureDateTime!).getTime() - new Date(b.departureDateTime!).getTime());
    }
    return flightsMap;
  }, [currentDate, flightLegs]);
  ...
  const handleDayHover = (e: React.MouseEvent<HTMLDivElement>, day: number) => {
    ...
    if (dayFlights && dayFlights.length > 0) {
      ...
      setTooltip({
        flights: dayFlights,
        position: { top: rect.top - (containerRect?.top || 0), left: rect.left + rect.width / 2 - (containerRect?.left || 0) },
      });
    }
  };
  ...
  const renderGridView = () => {
    ...
    const gridCells = [];
    dayNames.forEach(day => gridCells.push(<div key={day} className="text-center font-bold text-xs text-slate-500 dark:text-slate-400 py-2">{day}</div>));

    for (let i = 0; i < firstDayOfMonth; i++) gridCells.push(<div key={`empty-${i}`} className="rounded-lg h-16 sm:h-20" />);
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        >
          <span className={`text-sm font-semibold ${isToday ? 'bg-indigo-600 text-white rounded-full flex items-center justify-center w-6 h-6' : 'text-slate-700 dark:text-slate-200'}`}>{day}</span>
    ...
    return <div className="grid grid-cols-7 gap-1">{gridCells}</div>;
  };

  const renderAgendaView = () => agendaFlightsByDay.length > 0 ? (
    <div className="space-y-6">
      {agendaFlightsByDay.map(({ date, flights }) => (
        <div key={date.toString()}>
          <h3 className="font-bold text-lg text-slate-700 dark:text-slate-300 capitalize pb-2 mb-3 border-b border-slate-200 dark:border-slate-700">{formatFullDate(date)}</h3>
          <div className="space-y-3">
            {flights.map((leg) => (
              <div key={leg.id} className="flex items-center space-x-4 p-3 rounded-lg bg-slate-200/60 dark:bg-slate-700/60">
                <div className="flex-shrink-0"><AirlineLogo airline={leg.airline} size="sm" type="isotipo" /></div>
                <div className="flex-grow">
                    <div className="flex justify-between items-center">
                        <span className="font-bold text-slate-800 dark:text-slate-100">{leg.flightNumber}</span>
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${leg.type === 'ida' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200' : 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-200'}`}>
                    ...
                    <div className="flex items-center space-x-2 text-sm text-slate-600 dark:text-slate-400 mt-1">
                        ...
                        <span className="font-mono">{leg.departureAirportCode}</span>
                        <PlaneTakeoffIcon className="w-4 h-4" />
                        <span className="font-mono">{leg.arrivalAirportCode}</span>
                    </div>
                </div>
              </div>
            ))}
  ...
  return (
    <div ref={containerRef} className="relative bg-slate-100 dark:bg-slate-800 rounded-xl shadow-sm dark:shadow-none p-2 sm:p-4 md:p-6 min-h-[60vh]">
      <div className="flex justify-between items-center mb-6">
        ...
        <h2 className="text-lg sm:text-xl font-bold text-slate-800 dark:text-white capitalize text-center">{currentDate.toLocaleString('es-AR', { month: 'long', year: 'numeric' })}</h2>
      ...
      {tooltip && <TripTooltip flightLegs={tooltip.flights} position={tooltip.position} />}
    </div>
  );
};
