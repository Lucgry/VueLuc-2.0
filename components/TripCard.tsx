...
const TicketIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M16.5 6v.75m0 3v.75m0 3v.75m0 3V18m-9-1.5h5.25m-5.25 0h5.25m-5.25-2.25h5.25m-5.25 2.25h5.25M9 7.5a.75.75 0 0 1 .75-.75h4.5a.75.75 0 0 1 0 1.5h-4.5A.75.75 0 0 1 9 7.5Zm-5.25 9a.75.75 0 0 1 .75-.75h13.5a.75.75 0 0 1 0 1.5H4.5a.75.75 0 0 1-.75-.75Z"
    />
  </svg>
);
...
const FlightInfo: React.FC<{ flight: Flight; type: "Ida" | "Vuelta" }> = ({
  flight,
  type,
}) => {
      ...
      <div className="flex items-center justify-between space-x-2">
        <div className="text-center">
          <p className="text-2xl font-bold">
            {formatTime(flight.departureDateTime ?? null)}
          </p>
        ...
        <div className="text-center">
          <p className="text-2xl font-bold">
            {formatTime(flight.arrivalDateTime ?? null)}
          </p>
          <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">
...
const TripCard: React.FC<TripCardProps> = ({
  ...
  const [deletingStatus, setDeletingStatus] = useState<{
    ida: DeletionState;
    vuelta: DeletionState;
  }>({
    ida: "idle",
    vuelta: "idle",
  });
  const [tripDeletionState, setTripDeletionState] = useState<"idle" | "confirming">(
    "idle"
  );
    ...
    if (isExpanded) {
      ...
      const checkPasses = async () => {
        try {
          ...
          console.error(
            "Error checking for boarding passes, likely a permissions issue:",
            error
          );
      ...
      if (deletingStatus[flightType] === "confirming") {
        return (
          <div className="flex items-center space-x-2 bg-red-100 dark:bg-red-900/50 p-2 rounded-lg">
            <span className="text-sm font-semibold text-red-800 dark:text-red-200 flex-grow text-center">
              ¿Seguro?
            </span>
            <button
  ...
  const generateShareableText = (tripToShare: Trip): string => {
    ...
    if (ida) {
      ...
      text += `Sale ${ida.departureCity} (${ida.departureAirportCode}) a las ${formatTime(
        ida.departureDateTime ?? null
      )} hs\n`;
      text += `Llega ${ida.arrivalCity} (${ida.arrivalAirportCode}) a las ${formatTime(
        ida.arrivalDateTime ?? null
      )} hs\n\n`;
    }
    ...
    if (vuelta) {
      ...
      text += `Sale ${vuelta.departureCity} (${vuelta.departureAirportCode}) a las ${formatTime(
        vuelta.departureDateTime ?? null
      )} hs\n`;
      text += `Llega ${vuelta.arrivalCity} (${vuelta.arrivalAirportCode}) a las ${formatTime(
        vuelta.arrivalDateTime ?? null
      )} hs\n`;
    }
  ...
  // ✅ Agrupar manual: sólo si ambos son one-way y son legs opuestos
  ...
  const sourceNorm = sourceTrip
    ? normalizeTripFlights(sourceTrip)
    : { idaFlight: undefined, vueltaFlight: undefined };

  const sourceIsOneWay = sourceTrip
    ? ((!!sourceNorm.idaFlight) !== (!!sourceNorm.vueltaFlight))
    : false;

  const thisLeg: "ida" | "vuelta" | null =
    idaFlight && !vueltaFlight ? "ida" : vueltaFlight && !idaFlight ? "vuelta" : null;

  const sourceLeg: "ida" | "vuelta" | null =
    sourceNorm.idaFlight && !sourceNorm.vueltaFlight
      ? "ida"
      : sourceNorm.vueltaFlight && !sourceNorm.idaFlight
      ? "vuelta"
      : null;
        ...
        <div className={`p-4 ${!groupingState.active && "cursor-pointer"}`}>
          <div className="flex flex-col space-y-2">
            ...
            {status && status.type === "pill" && (
              <div
                className={`inline-flex items-center space-x-1.5 text-xs font-semibold px-2 py-1 rounded-full ${status.color} ${status.bg}`}
              >
