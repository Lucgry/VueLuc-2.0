...
const YearSeparator: React.FC<{ year: number }> = ({ year }) => (
  <div className="flex items-center space-x-4 my-6" aria-hidden="true">
    ...
    <span className="font-bold text-lg text-slate-400">{year}</span>
    <div className="flex-1 h-px bg-slate-700/50"></div>
  </div>
);
...
const TripList: React.FC<TripListProps> = ({ trips, onDeleteTrip, listFilter, nextTripId, userId }) => {
  if (trips.length === 0) {
    ...
    return (
      <div className="text-center py-20 px-6 bg-slate-800/50 backdrop-blur-md border border-slate-700/50 rounded-xl">
        {icon}
