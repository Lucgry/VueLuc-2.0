...
import { getBoardingPass, saveBoardingPass, moveBoardingPass, deleteBoardingPassesForTrip } from "./services/db";

import { onAuthStateChanged, User } from "firebase/auth";
import { db, auth, isFirebaseInitialized, firebaseInitializationError, projectId } from "./firebase";
import { collection, addDoc, deleteDoc, doc, onSnapshot, query, orderBy, updateDoc } from "firebase/firestore";

// Type guard for BeforeInstallPromptEvent
      ...
      <div className="mt-6 p-4 rounded-lg bg-red-100/50 dark:bg-red-900/20 text-left flex items-start space-x-3 shadow-neumo-light-in dark:shadow-neumo-dark-in">
        ...
        <div>
          ...
          <p className="text-sm text-red-700 dark:text-red-300 mt-3">
            La causa más común es que tu clave de API de Firebase tenga **Restricciones de sitios web** que bloquean esta aplicación.
          </p>
          ...
          <ol className="list-decimal list-inside text-sm text-red-700 dark:text-red-300 mt-2 space-y-2">
            ...
            <li>
              Asegúrate de que **TODAS** las siguientes URLs estén en la lista de sitios permitidos:
              <ul className="list-disc list-inside pl-5 mt-1 space-y-1">
                <li>
                  <code className="text-xs bg-red-200/50 dark:bg-red-800/30 p-0.5 rounded">https://vueluc-2.netlify.app/*</code>
                </li>
                <li>
                  <code className="text-xs bg-red-200/50 dark:bg-red-800/30 p-0.5 rounded">
...
const App: React.FC = () => {
  // Hooks SIEMPRE arriba, sin returns previos (evita React #310)
        ...
        currentUser
          ...
          .catch((error) => {
            ...
            const isHttpError =
              ...
              errorMessage.includes("securetoken") ||
  ...
  const mergeTrips = useCallback(
    async (idaTrip: Trip, vueltaTrip: Trip) => {
      if (!user || !db) return;
      ...
      if ((vueltaPass as any).exists && (vueltaPass as any).file) {
        await saveBoardingPass(user.uid, idaTrip.id, "vuelta", (vueltaPass as any).file);
      }
  ...
  const handleSplitTrip = useCallback(
    async (trip: Trip) => {
      if (!user || !db) return;
      ...
      try {
        ...
        await updateDoc(originalTripRef, { returnFlight: null });
      } catch (error) {
  ...
  const runAutomaticGrouping = useCallback(
    async (currentTrips: Trip[]) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (querySnapshot) => {
        ...
        querySnapshot.forEach((d) => tripsFromFirestore.push({ id: d.id, ...(d.data() as any) } as Trip));
  ...
  // 3) One-time duplicate cleanup
  useEffect(() => {
    if (!user || !db || trips.length === 0 || duplicateCleanupRun.current) return;

    const runDuplicateCleanup = async () => {
      duplicateCleanupRun.current = true;

      const seenFingerprints = new Set<string>();
      const promises: Promise<void>[] = [];

      const sortedForCleanup = [...trips].sort((a, b) => new Date(a.createdAt || 0).getTime() - new Date(b.createdAt || 0).getTime());
      ...
      for (const trip of sortedForCleanup) {
        ...
        if (updates.departureFlight) {
          const fp = getFlightFingerprint(updates.departureFlight);
        ...
        if (updates.returnFlight) {
          const fp = getFlightFingerprint(updates.returnFlight);
          if (fp && seenFingerprints.has(fp)) {
  ...
  const handleInstallClick = () => {
    ...
    installPromptEvent.userChoice.then((choiceResult) => {
      if (choiceResult.outcome === "accepted") setIsInstallBannerVisible(false);
      setInstallPromptEvent(null);
    });
  };

  const onStartGrouping = (trip: Trip) => setGroupingState({ active: true, sourceTrip: trip });
  ...
  const filteredTrips = useMemo<Trip[]>(() => {
    return trips.filter((trip) => {
      ...
      if (listFilter === "future") return !!endDate && endDate >= now;
      if (listFilter === "completed") return !!endDate && endDate < now;
      if (listFilter === "currentMonth") return !!startDate && startDate.getMonth() === now.getMonth() && startDate.getFullYear() === now.getFullYear();
      return true;
    });
  }, [trips, listFilter]);
  ...
  const sortedTrips = useMemo<Trip[]>(() => {
    return [...filteredTrips].sort((a, b) => {
      ...
      if (listFilter === "completed") return dateB.getTime() - dateA.getTime();
      return dateA.getTime() - dateB.getTime();
    });
  }, [filteredTrips, listFilter]);

  const nextTripInfo = useMemo<{ id: string | null }>(() => ({ id: nextTrip ? nextTrip.id : null }), [nextTrip]);

  // Render states (DESPUÉS de hooks)
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme === "dark" ? "dark" : ""}`}>
      <div className="max-w-4xl mx-auto p-4 sm:p-6 pb-24">
        <Header theme={theme} onToggleTheme={toggleTheme} isAirportMode={isAirportMode} onToggleAirportMode={() => setIsAirportMode(true)} />
        ...
        {isInstallBannerVisible && <InstallBanner onInstall={handleInstallClick} onDismiss={() => setIsInstallBannerVisible(false)} />}
        ...
        <div className="flex justify-center mb-6">
          <div className="bg-slate-200 dark:bg-slate-700/50 p-1 rounded-xl grid grid-cols-3 gap-1 w-full max-w-md shadow-inner">
            <button
              ...
              className={`flex items-center justify-center py-2 rounded-lg text-sm font-semibold transition-all ${
                view === "list"
        ...
        {view === "list" && (
          <>
            {nextTrip && nextTripFlight && listFilter === "future" && <NextTripCard flight={nextTripFlight} flightType={nextTripFlightType} />}
                ...
                <button
                  ...
                  className={`flex-shrink-0 whitespace-nowrap px-4 py-1.5 rounded-full text-sm font-medium transition-all border ${
                    listFilter === option.key
                      ...
                      : "bg-transparent border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
                  }`}
                >
                  {option.label}
                </button>
        ...
        <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end space-y-3">
          ...
          <button
            ...
            className={`p-4 rounded-full shadow-2xl text-white transition-transform duration-300 ${
              isFabMenuOpen ? "bg-slate-600 rotate-45" : "bg-gradient-to-r from-indigo-600 to-purple-600 hover:scale-110"
            }`}
        ...
        {isModalOpen && (
          <EmailImporter
            ...
            onAddTrip={onAddTrip}
            // Ya NO se usa apiKey en frontend. Se pasa vacío por compatibilidad con el componente.
            apiKey={""}
            onInvalidApiKey={() => {
              /* no-op */
            }}
        ...
        {isQuickAddModalOpen && <QuickAddModal onClose={() => setIsQuickAddModalOpen(false)} onAddTrip={onAddTrip} />}
      </div>
    </div>
  );
};
